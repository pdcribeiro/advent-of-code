...
func moveCrates(file *utils.File, stacks []Stack) {
	for file.ReadLineOld() {
		...
		crates := popCrates(&stacks[from], count)
		pushCratesInReverse(&stacks[to], crates)
	}
}

func popCrates(stack *Stack, count int) (crates []string) {
	for i := 0; i < count; i++ {
...
func pushCratesInReverse(stack *Stack, crates []string) {
